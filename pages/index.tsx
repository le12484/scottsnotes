...
import {QueryResults} from '../types/documents';
import {UpdateRequest} from './api/update';
...
export default function Conversation() {
    ...
    const {apiKey, chatModelId, systemPurposeId, setSystemPurpose} =
    ...
    async function handleListDelete(uid: string) {
        const message = messages.find(message => message.uid === uid);
        if (message?.memoryId) {
            await deleteNote(message.memoryId);
        }
    ...
    async function handleListEdit(uid: string, newText: string) {
        setMessages(list =>
            list.map(message =>
                message.uid === uid ? {...message, text: newText} : message,
            ),
        );

        const message = messages.find(message => message.uid === uid);
        if (message?.memoryId) {
            console.log('memoryId', message.memoryId, 'newText', newText);
            await updateNotes(message.memoryId, newText);
        }
    }
    ...
    const handleListRunAgain = (uid: string) => {
        ...
        getBotMessageStreaming(conversation).then(() =>
    ...
    const updateNotes = async (uid: string, text: string) => {
        ...
        if (response.body) {
            ...
            while (true) {
                ...
                try {
                    ...
                    console.log('Error parsing JSON for messageText: ' + e);
                }
                ...
                if (
                    !newBotMessage.model &&
                    newBotMessage.text.startsWith('{')
                ) {
                    const endOfJson = newBotMessage.text.indexOf('}');
                    if (endOfJson > 0) {
                        const json = newBotMessage.text.substring(
                            0,
                            endOfJson + 1,
                        );
                        try {
                            ...
                            newBotMessage.text = newBotMessage.text.substring(
                                endOfJson + 1,
                            );
                        } catch (e) {
                            // error parsing JSON, ignore
    ...
    const handleComposerSendMessage: (text: string) => void = text => {
        ...
        if (!conversation.length) {
            const systemMessage = SystemPurposes[
                systemPurposeId
        ...
        >
            ...
            >
                ...
                >
                    ...
                    >
                        <NoSSR>
                            {GptChatModels[chatModelId]?.title ||
                                'Select Model'}{' '}
                            ...
                            <Box>
                                ...
                                <NoSSR>
                                    <Select
                                        ...
                                        onChange={(e, v) =>
                                            handlePurposeChange(v)
                                        }
                                        sx={{minWidth: '40vw'}}
                                    >
                                        {Object.keys(SystemPurposes).map(
                                            spId => (
                                                <Option key={spId} value={spId}>
                                                    {
                                                        SystemPurposes[
                                                            spId as SystemPurposeId
                                                        ]?.title
                                                    }
                                                </Option>
                                            ),
                                    ...
                                    >
                                        {
                                            SystemPurposes[systemPurposeId]
                                                .description
                                        }
                                    </Typography>
                                </NoSSR>
