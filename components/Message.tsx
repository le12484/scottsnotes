...
const inferLanguage = (
    ...
    if (code.startsWith('<DOCTYPE html') || code.startsWith('<!DOCTYPE'))
        return 'html';
    if (code.startsWith('<')) return 'xml';
    if (code.startsWith('from ')) return 'python';
    if (code.startsWith('import ') || code.startsWith('export '))
        return 'typescript'; // or python
    if (code.startsWith('interface ') || code.startsWith('function '))
        return 'typescript'; // ambiguous
...
function CodeBlock({
    ...
    const showRunIcon =
        codeBlock.complete &&
        !!codeBlock.language &&
        runnableLanguages.includes(codeBlock.language);
    ...
    return (
        ...
        >
            ...
            {showRunIcon && showSandpack && (
                <RunnableCode codeBlock={codeBlock} theme={theme} />
            )}
        </Box>
    );
}
...
function explainErrorInMessage(message: UiMessage) {
    ...
    const isAssistantError =
        ...
        (message.text.startsWith('Error: ') ||
            message.text.startsWith('OpenAI API error: '));
    if (isAssistantError) {
        if (
            message.text.startsWith('OpenAI API error: 429 Too Many Requests')
        ) {
            // TODO: retry at the api/chat level a few times instead of showing this error
            errorMessage = (
                <>
                    The model appears to be occupied at the moment. Kindly
                    select <b>GPT-3.5 Turbo</b> via settings icon, or give it
                    another go by selecting <b>Run again</b> from the message
                    menu.
                </>
            );
            ...
            errorMessage = (
                <>
                    Your API key appears to be unauthorized for{' '}
                    {message.model || 'this model'}. You can change to{' '}
                    <b>GPT-3.5 Turbo</b> via the settings icon and
                    simultaneously{' '}
                    <Link
                        noLinkStyle
            ...
            const usedText = match
                ? ` (${match[2]} tokens, max ${match[1]})`
                : '';
            errorMessage = (
                <>
                    This thread <b>surpasses the maximum size</b> allowed for{' '}
                    {message.model || 'this model'}
                    {usedText}. Please consider removing some earlier messages
                    from the conversation, start a new conversation, choose a
                    model with larger context, or submit a shorter new message.
                </>
            );
        }
    }
...
export function Message(props: {
    ...
    const chatFontCss = {
        my: 'auto',
        fontFamily:
            message.role === 'assistant' ? theme.fontFamily.code : undefined,
    ...
    return (
        ...
        >
            ...
            <Stack
                sx={{alignItems: 'center', minWidth: 64, textAlign: 'center'}}
            ...
            >
                {isHovering ? (
                    ...
                    <MemoryIcon sx={{width: 40, height: 40}} />
                ...
                {!!menuAnchor && (
                    ...
                    >
                        ...
                        <MenuItem onClick={handleMenuSave}>
                            <ListItemDecorator>
                                <SaveIcon />
                            </ListItemDecorator>
                            Save
            ...
            {isEditing ? (
                <Textarea
                    ...
                    sx={{...chatFontCss, flexGrow: 1, marginLeft: 8}}
                />
            ) : (
                <Box
                    sx={{
                ...
                >
                    {parseAndHighlightCodeBlocks(collapsedText).map(
                        (part, index) =>
                            part.type === 'code' ? (
                                <CodeBlock
                                    key={index}
